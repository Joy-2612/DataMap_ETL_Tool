...
const FlowDiagrams = () => {
  ...
  const handleActionOperationsOnRun = async (actionNode) => {
    ...
    const datasetResponse = await fetch(`http://localhost:5000/api/file/dataset/${data.newFileId}`);
  ...
  return (
    <ReactFlowProvider>
      <div className={styles.runButtonContainer}>
          ...
          <button onClick={handleRemoveFlowDiagram} className={styles.removeButton}>
          Clear Flow Diagram
        </button>
        </div>
