...
const RightSideBar = ({ onAddNode, onAddNodeOutput, userId, selectedEdge, setEdges }) => {
  ...
  const handleActionClick = (action) => {
    if (selectedEdge) {
      ...
      setEdges((prevEdges) =>
        prevEdges.map((edge) => (edge.id === selectedEdge.id ? updatedEdge : edge))
      );
    }
  };
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <div
            ...
            className={`${styles.draggableItem} ${selectedItem?.id === item.id ? styles.selectedItem : ''}`}
          >
            {item.name}
          </div>
        ))
  ...
  const renderActionOptions = () => (
    <div className={styles.actionOptions}>
      <div className={styles.actionOption} onClick={() => handleActionClick('Concatenate')}>Concatenate</div>
      <div className={styles.actionOption} onClick={() => handleActionClick('Split')}>Split</div>
      <div className={styles.actionOption} onClick={() => handleActionClick('Merge')}>Merge</div>
      <div className={styles.actionOption} onClick={() => handleActionClick('Standardize')}>Standardize</div>
      <div className={styles.actionOption} onClick={() => handleActionClick('Convert to CSV')}>Convert to CSV</div>
      <div className={styles.actionOption} onClick={() => handleActionClick('Convert to XML/JSON')}>Convert to XML/JSON</div>
    </div>
  );
    ...
    <div className={styles.addNodeContainer}>
      ...
      {newNodeName && (
        <button
          ...
          onClick={() => {
            if (onAddNodeOutput) onAddNodeOutput({ id: Date.now().toString(), name: newNodeName });
  ...
  return (
    <div className={styles.sidebar}>
      <div className={styles.tabHeader}>
        ...
        {selectedEdge && (
          <div
            className={`${styles.tab} ${activeTab === 'edgeActions' ? styles.activeTab : ''}`}
